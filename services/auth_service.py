...
async def send_password_reset_otp(email: str, db: Session):
    ...
    send_attempts_key = f"password_reset_send_attempts:{email}"
    send_attempts = await redis_client.get(send_attempts_key)
    send_attempts = int(send_attempts) if send_attempts else 0
    if send_attempts >= OTP_SEND_LIMIT:
        raise HTTPException(
            status_code=status.HTTP_429_TOO_MANY_REQUESTS,
            detail="Too many OTP requests. Please try again later.",
        )
    await redis_client.set(send_attempts_key, send_attempts + 1, ex=OTP_EXPIRE_TIME)
    send_email(
        to_email=email,
